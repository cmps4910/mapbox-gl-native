...
public:
    ...
    static AttributeBinding attributeBinding(const EvaluatedType& currentPropertyValue,
                                             const EvaluatedType& lastLayoutPropertyValue,
                                             const optional<VertexBuffer>& vertexBuffer) {
        // If the current evaluated property has a constant value, use that.
        // Otherwise, if the evaluated property at the time of the last layout
        // has a constant value, use that. Otherwise, the last layout must have
        // generated a paint vertex buffer from the variable value; use that.
        if (currentPropertyValue.isConstant()) {
            return typename Attribute::ConstantBinding {
                Attribute::value(*currentPropertyValue.constant())
            };
        } else if (lastLayoutPropertyValue.isConstant()) {
            return typename Attribute::ConstantBinding {
                Attribute::value(*lastLayoutPropertyValue.constant())
            };
        } else {
            assert(vertexBuffer);
            return gl::Attributes<Attribute>::allVariableBindings(*vertexBuffer)
